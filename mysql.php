...
// $data = array(
//     'openId'     => 'jack',
//     'unionid'    => 'dddddddddccsfazzd',
//     'nickname'   => 'kj',
//     'sex'        => '1',
//     'headimgurl' => 'http://ddd.com/logo.png'
// );
// $db = new mysql();
// var_dump($db->table('user')->insert($data));
class mysql
{
    ...
    public function __construct($_config='')
    {
        ...
        if(!$link)
        {
            $db = @mysqli_connect($this->_config['hostname'],$this->_config['username'],$this->_config['password'],$this->_config['dbname']);
            if(!$db)
            {
                $this->ShowException("Error: ".mysqli_connect_error());
                exit;
            }
    ...
    public function execute($sql=null)
    {
        ...
        if(!$res)
        {
            ...
            $this->ShowException("SQL执行出错！<br/>错误号：".$errors[0]['errno']."<br/>SQL错误状态：".$errors[0]['sqlstate']."<br/>错误信息：".$errors[0]['error']);
            die();
        }
        return $res;
    }
